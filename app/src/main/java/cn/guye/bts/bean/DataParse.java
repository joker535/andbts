...
public class DataParse {
    ...
    public void parseKLine(String quoteId, BucketObject[] bucketObjects){
        HistoryPrice[] historyPrice = getPrice(bucketObjects,quoteId);
        Asset base = (Asset) BtsContorler.getInstance().getDataSync("1.3.113");
        Asset quote = (Asset) BtsContorler.getInstance().getDataSync(quoteId);
        ArrayList<KLineBean> kLineBeans = new ArrayList<>();
        float vol = 0;
        SimpleDateFormat dateFormat = new SimpleDateFormat("MM-dd");
        for (int i = 0 ; i < historyPrice.length ; i++){
            if(historyPrice[i] == null){
                continue;
            }

            float high ,low , open , close;
            if(historyPrice[i].high.base.getAsset().getObjectId().equals("1.3.113")){
                high = historyPrice[i].high.base2Quote(base,quote).floatValue();
                low = historyPrice[i].low.base2Quote(base,quote).floatValue();
                open = historyPrice[i].open.base2Quote(base,quote).floatValue();
                close = historyPrice[i].close.base2Quote(base,quote).floatValue();
                vol = Price.get_asset_amount(bucketObjects[i].quote_volume,quote).floatValue();
            }else{
                high = historyPrice[i].high.quote2Base(quote,base).floatValue();
                low = historyPrice[i].low.quote2Base(quote,base).floatValue();
                open = historyPrice[i].open.quote2Base(quote,base).floatValue();
                close = historyPrice[i].close.quote2Base(quote,base).floatValue();
                vol = Price.get_asset_amount(bucketObjects[i].base_volume,quote).floatValue();
            }
            ...
            if (high == Double.NaN || high == Double.POSITIVE_INFINITY) {
                high = findMax(open, close);
            }
            if (close == Double.POSITIVE_INFINITY || close == 0) {
                close = open;
            }
            if (open == Double.POSITIVE_INFINITY || open == 0) {
                open = close;
            }
            ...
            KLineBean kLineData = new KLineBean();
            kLineData.date = dateFormat.format(historyPrice[i].date);
            kLineData.open = open;
    ...
    private HistoryPrice[] getPrice(BucketObject[] bucketObjects, String quoteId) {
        Asset base = (Asset) BtsContorler.getInstance().getDataSync("1.3.113");
        Asset quote = (Asset) BtsContorler.getInstance().getDataSync(quoteId);
        Map<String , Asset> assets = new HashMap<>();
        assets.put(base.getObjectId(),base);
        assets.put(quote.getObjectId(),quote);
        HistoryPrice[] prices = new HistoryPrice[bucketObjects.length];

        for (int i = 0 ; i< bucketObjects.length ; i++){
            BucketObject last = bucketObjects[i];

            if(last.high_base.equals(BigDecimal.ZERO) || last.low_base.equals(BigDecimal.ZERO) || last.open_base.equals(BigDecimal.ZERO) || last.close_base.equals(BigDecimal.ZERO)
                    || last.high_quote.equals(BigDecimal.ZERO) || last.low_quote.equals(BigDecimal.ZERO) || last.open_quote.equals(BigDecimal.ZERO) || last.close_quote.equals(BigDecimal.ZERO)){
                prices[i] = null;
                continue;
            }

            prices[i]= new HistoryPrice();
            prices[i].high = new Price();
            prices[i].high.base = new AssetAmount(last.high_base,assets.get(last.key.base.getObjectId()));
            prices[i].high.quote = new AssetAmount(last.high_quote,assets.get(last.key.quote.getObjectId()));

            prices[i].low = new Price();
            prices[i].low.base = new AssetAmount(last.low_base,assets.get(last.key.base.getObjectId()));
            prices[i].low.quote = new AssetAmount(last.low_quote,assets.get(last.key.quote.getObjectId()));

            prices[i].open = new Price();
            prices[i].open.base = new AssetAmount(last.open_base,assets.get(last.key.base.getObjectId()));
            prices[i].open.quote = new AssetAmount(last.open_quote,assets.get(last.key.quote.getObjectId()));

            prices[i].close = new Price();
            prices[i].close.base = new AssetAmount(last.close_base,assets.get(last.key.base.getObjectId()));
            prices[i].close.quote = new AssetAmount(last.close_quote,assets.get(last.key.quote.getObjectId()));

            prices[i].volume = new Price();
            prices[i].volume.base = new AssetAmount(last.base_volume,assets.get(last.key.base.getObjectId()));
            prices[i].volume.quote = new AssetAmount(last.quote_volume,assets.get(last.key.quote.getObjectId()));

            prices[i].date = last.key.open;
        }
