package cn.guye.bitshares.models.backup;
...
import cn.guye.bitshares.crypto.SecureRandomGenerator;
...
public class Wallet {
    ...
    public Wallet(String name, String brainKey, int brainkeySequence, String chainId, String password){
        ...
        byte[] passwordHash = sha256_object.create_from_string(password).hash;
        this.password_pubkey = new Address(ECKey.fromPublicOnly(ECKey.fromPrivate(passwordHash).getPubKey())).toString();
        byte[] brainkeyHash = sha256_object.create_from_string(brainKey).hash;
